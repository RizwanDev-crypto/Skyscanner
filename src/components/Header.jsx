...
import LanguageIcon from "@mui/icons-material/Language"; // Not exact match, but close
import HelpOutlineIcon from "@mui/icons-material/HelpOutline"; // Not exact match
...
const pages = [
  { label: "Hotel", icon: <RadioButtonUncheckedIcon sx={{ fontSize: "20px" }} />, active: true },
  { label: "Car Rental", icon: <RadioButtonUncheckedIcon sx={{ fontSize: "20px" }} />, active: false },
  { label: "Events", icon: <RadioButtonUncheckedIcon sx={{ fontSize: "20px" }} />, active: false },
  { label: "Tours", icon: <RadioButtonUncheckedIcon sx={{ fontSize: "20px" }} />, active: false },
  { label: "Visa", icon: <WalletIcon sx={{ fontSize: "20px" }} />, active: false },
];

// Helper to convert emoji flag to country code (e.g., 🇦🇫 -> af)
...
export default function Header() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const [cntRes, langRes, currRes] = await Promise.all([
          fetch('/api/countries'),
          fetch('/api/languages'),
        ...
        setCountries(cntData.countries);
        setLanguages(langData.languages);
        setCurrencies(currData.currencies);

        // Set defaults
        setSelectedCountry(cntData.countries.find(c => c.name === "Pakistan") || cntData.countries[0]);
        setSelectedLanguage(langData.languages.find(l => l.code === "EN") || langData.languages[0]);
        setSelectedCurrency(currData.currencies.find(c => c.code === "AOA") || currData.currencies[0]);
  ...
  return (
    <AppBar
      position="static" // Changed from sticky to static for now, as sticky might need offset handling
      elevation={0}
      sx={{
    ...
    >
      <Container maxWidth="lg">
        ...
        <Toolbar disableGutters sx={{ minHeight: { xs: 56, md: 75 } , justifyContent: "space-between"}}>
            {/* Logo */}
            <Box sx={{  display: "flex", alignItems: "center" }}>
              <a href="/" style={{ display: 'flex', alignItems: 'center' }}>
                <svg
                  ...
                  viewBox="0 0 910 169"
                  aria-hidden="true"
                  style={{ height: 32, width: 'auto', fill: '#fff' }}
                >
            ...
            <Box sx={{ display: { xs: "none", md: "flex" }, alignItems: "center", gap: 4 }}>
                ...
                >
                   {selectedCountry && (
                    <img 
                      src={`https://flagcdn.com/w40/${getCountryCode(selectedCountry.icon)}.png`}
                      width="20" 
                      height="15" 
                    ...
                    <Box sx={{ borderLeft: "2px solid rgba(255,255,255,0.7)", height: "18px", mx: 0.5 }} />
                    <Typography 
                      onClick={(e) => {
                        e.stopPropagation();
                        setOpenLanguageModal(true);
                      }}
                      sx={{ fontSize: "16px", fontWeight: "600", cursor: "pointer", "&:hover": { color: "#00a698" } }}
                    >
        ...
        {/* BOTTOM ROW: Navigation Pills - Redesigned as Glassmorphism Pill */}
        <Box sx={{ display: "flex", justifyContent: "center", width: "100%", mb: 2 }}>
          ...
          >
              {pages.map((page) => {
                const isActive = activeTab === page.label;
                return (
                  <Button
                      key={page.label}
                      startIcon={page.icon}
                      ...
                      "&:hover": {
                          backgroundColor: isActive ? "#eee" : "rgba(255, 255, 255, 0.15)",
                      },
                      }}
                  >
                      {page.label}
      ...
      >
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pb: 1 }}>
          <Typography variant="h6" sx={{ fontWeight: "bold" }}>Country/Region</Typography>
          <IconButton onClick={() => setOpenCountryModal(false)} size="small">
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent>
          <TextField
            ...
            sx={{
              ...
              "& .MuiOutlinedInput-root": {
                ...
                "&.Mui-focused fieldset": { borderColor: "#00a698" }, // Skyscanner green
              },
            }}
            InputProps={{
              startAdornment: (
          ...
          >
            {filteredCountries.map((country) => (
              ...
              >
                <Box sx={{ width: 24, height: 18, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                  {country.icon && (
                    <img 
                      src={`https://flagcdn.com/w40/${getCountryCode(country.icon)}.png`}
                      width="20"
                      alt={country.name}
                      style={{ borderRadius: '2px', objectFit: 'cover' }}
                      onError={(e) => {
      ...
      >
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pb: 1 }}>
          ...
          <IconButton onClick={() => setOpenLanguageModal(false)} size="small">
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent>
      ...
      >
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pb: 1 }}>
          ...
          <IconButton onClick={() => setOpenCurrencyModal(false)} size="small">
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent>
